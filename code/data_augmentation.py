...
def aug_paraphrase(df):
    ...
    # 5. save
    # concated_df.to_csv("~/data/train_splitted_clean_correct_paraphrased.csv", index=False)
    # aug_df.to_csv("~/data/train_splitted_clean_correct_parap_onlyaug.csv", index=False)
...
def data_swap(df):
    ...
    # concated_df.to_csv("~/data/train_splitted_clean_correct_paraphrased_swap.csv")
    return concated_df


if __name__ == "__main__":
    train_path = '~/data/train_splitted_clean_correct.csv'
    dev_path = '~/data/dev_splitted_clean_correct.csv'
    ...
    train_paraphrased.to_csv("~/data/train_splitted_clean_correct_paraphrased.csv", index=False)
    
    dev_paraphrased, dev_parap_onlyaug = aug_paraphrase(dev_data)
    train_paraphrased.to_csv("~/data/dev_splitted_clean_correct_paraphrased.csv", index=False)
    
    
    # paraphrasing + swapping
    train_paraphrased_swap = data_swap(train_paraphrased)
    train_paraphrased_swap.to_csv("~/data/train_splitted_clean_correct_paraphrased_swap.csv")
    
    dev_paraphrased_swap = data_swap(dev_paraphrased)
    dev_paraphrased_swap.to_csv("~/data/dev_splitted_clean_correct_paraphrased_swap.csv")