...
if __name__ == '__main__':
    ...
    parser.add_argument('--project_name', default="STS_resample_swap_v2_sweep")
    ...
    def sweep_train(config=None):
        with wandb.init(config=config) as run:
            config = wandb.config
            run.name = f"{config.model_name}_{config.loss_func}_{config.learning_rate}_{config.batch_size}_{config.weight_decay}_{config.max_epoch}_{config.weight_decay}_steplr"
            ...
            wandb_logger = WandbLogger(
                ...
                name=f"{config.model_name}_{config.loss_func}_{config.learning_rate}_{config.batch_size}_{config.weight_decay}_{config.max_epoch}_{config.weight_decay}_steplr"
            )
